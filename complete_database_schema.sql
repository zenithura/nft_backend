...
-- We use DO blocks to check if types exist before creating them
...
CREATE TABLE IF NOT EXISTS users (
    ...
    -- Authentication fields (from frontend registration form)
    email VARCHAR(255) NOT NULL UNIQUE,
    password_hash VARCHAR(255) NOT NULL,
    
    -- User profile fields (from frontend registration form)
    username VARCHAR(100),
    first_name VARCHAR(100),
    last_name VARCHAR(100),
    
    -- Role selection: BUYER or ORGANIZER (required during signup)
    -- Also supports ADMIN, RESELLER, SCANNER for future use
    ...
    -- Optional wallet address reference (for Web3 integration)
...
-- Note: These allow all operations for backend service role
...
-- ============================================================================
-- PART 7: TABLE COMMENTS FOR DOCUMENTATION
-- ============================================================================

-- Authentication tables
COMMENT ON TABLE users IS 'User accounts with authentication and profile information';
COMMENT ON COLUMN users.email IS 'User email address (unique, used for login)';
COMMENT ON COLUMN users.password_hash IS 'Bcrypt/Argon2 hashed password (never store plain text)';
COMMENT ON COLUMN users.role IS 'User role: BUYER (default) or ORGANIZER (selected during signup)';
...
COMMENT ON COLUMN users.is_active IS 'Whether account is active (can be disabled by admin)';
...
COMMENT ON COLUMN refresh_tokens.token IS 'Refresh token string (hashed or encrypted)';
COMMENT ON COLUMN refresh_tokens.expires_at IS 'Token expiration timestamp';
COMMENT ON COLUMN refresh_tokens.is_valid IS 'Whether token is still valid (can be revoked)';
...
COMMENT ON TABLE allowlist IS 'Wallet allowlist for events';
COMMENT ON TABLE metrics IS 'Event metrics and analytics';
COMMENT ON TABLE bot_detection IS 'Bot detection and fraud prevention records';
COMMENT ON TABLE payouts IS 'Payment payouts to organizers, sellers, etc.';

-- ============================================================================
-- PART 8: SECURITY & ADMIN TABLES
-- ============================================================================
...
COMMENT ON COLUMN security_alerts.status IS 'Alert status: NEW, REVIEWED, BANNED, IGNORED';
...
COMMENT ON TABLE user_activity_logs IS 'Log of all user activities for audit trail';
COMMENT ON TABLE admin_actions IS 'Log of all admin actions for audit trail';

-- ============================================================================
-- VERIFICATION QUERIES (Uncomment to run after schema creation)
-- ============================================================================
